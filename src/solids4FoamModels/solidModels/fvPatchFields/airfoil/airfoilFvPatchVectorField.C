...
#include "airfoilFvPatchVectorField.H"
    ...
    curTimeIndex_(-1)
{
    ...
    )
    {
        FatalErrorIn
        (
            "airfoilFvPatchVectorField::airfoilFvPatchVectorField"
    ...
    if (dict.found("pressureDistribution"))
    {
        flowPressure_ = interpolationTable<scalar>(dict.subDict("pressureDistribution"));
    }
    else
    {
        FatalErrorIn
        (
             "airfoilFvPatchVectorField::airfoilFvPatchVectorField"
        )    << "pressureDistribution file should be provided"
        << abort(FatalError);
    }
...
airfoilFvPatchVectorField
(
    const airfoilFvPatchVectorField& pvf,
    const fvPatch& p,
    const DimensionedField<vector, volMesh>& iF,
...
:
    ...
    tractionFieldPtr_(),
    pressureFieldPtr_(),
    flowPressure_(pvf.flowPressure_),
...
void airfoilFvPatchVectorField::autoMap
(
...
void airfoilFvPatchVectorField::rmap
(
...
)
{
    ...
    const airfoilFvPatchVectorField& rpvf =
        refCast<const airfoilFvPatchVectorField>(pvf);
...
void airfoilFvPatchVectorField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    scalarField flowPressure(press.size(), 0.0);

    const vectorField& C = this->patch().Cf();

    forAll(flowPressure, faceI)
    {
        flowPressure[faceI] = flowPressure_(C[faceI].x());
    }
...
void airfoilFvPatchVectorField::evaluate
(
...
void airfoilFvPatchVectorField::write(Ostream& os) const
{
...
makePatchTypeField(fvPatchVectorField, airfoilFvPatchVectorField);
